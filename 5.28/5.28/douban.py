...
class DoubanSpider:
    ...
    def run(self):
        df = pd.DataFrame(
            columns=["序号", "标题", "链接", "导演", "主演", "年份", "国家", "类型", "评分", "评价人数", "简介"])
        count = 1  # 用来计数
        for url in self.urls:
            ...
            for li in lis:
                ...
                df.loc[len(df.index)] = [count, title, src, director, actors, year, country, type, score, comment, summary]
                count += 1
        df.to_excel(self.file_name + ".xlsx", sheet_name=self.file_name, index=False)
    ...
    def visualization(self):
        ...
        ## 图2. 绘制电影 国家 饼形图 ，只获取前 20 个份额最大的国家，剩下的用 其他 表示，一个电影可能包含多个国家，国家列中使用空格分开的
        df_copy = df.copy()  # 拷贝一份数据
        front = 15
        ...
        ## 图4. 绘制电影评分和评价人数之间的散点图，比较他们之间的关系。
        df_copy = df.copy()  # 拷贝一份数据
